...
until [ "$(psql -h postgres -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -tAc "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('work_experience', 'education', 'projects', 'skills', 'certificates', 'cv_generator_preferences');")" = "6" ]; do
  sleep 2
done
